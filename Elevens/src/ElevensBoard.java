...
public class ElevensBoard extends Board {
		...
		for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
			...
			for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
				...
				if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
					cardsFound.add(new Integer(k1));
					cardsFound.add(new Integer(k2));
					return cardsFound;
				}
			}
		}
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
		...
		if(foundJack>-1 && foundQueen>-1 && foundKing>-1){
			cardsFound.add(new Integer(foundJack));
			cardsFound.add(new Integer(foundQueen));
			cardsFound.add(new Integer(foundKing));
		}
		return cardsFound;
	}
	...
	public boolean playIfPossible() {
		...
		List<Integer> playableCards = findPairSum11(positions);
		if(playableCards.size() == 2){
			replaceSelectedCards(playableCards);
			return true;
		}
		playableCards = findJQK(cardIndexes());
